...
function tokenizeDart(code) {
  const regex = /(\/\/.*)|('(?:\\'|[^'])*'|"(?:\\"|[^"])*")|(@\w+)|(\b(?:import|class|extends|void|return|const|super|final|var|true|false|null|override|import|const|package)\b)|(\b[A-Z]\w*\b)|(\b\d+(?:\.\d+)?\b)/g;
  ...
  while ((match = regex.exec(code)) !== null) {
    ...
    if (match.index > lastIndex) {
      tokens.push({ text: code.substring(lastIndex, match.index), type: 'plain' });
    }
    
    const [lexeme, comment, string, annotation, keyword, className, number] = match;
